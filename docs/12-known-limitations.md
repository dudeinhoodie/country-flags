...
- **Прогресс переносится через первую синхронизацию, но карточку, которой нет в пришедшем релизе, перенести некуда.** Приложение засевает пустое хранилище зашитым каталогом ([ADR-021](./adr/ADR-021-bundled-catalogue-snapshot.md)) под собственными идентификаторами, и первая же успешная синхронизация вытесняет засев целиком. В момент вытеснения клиент сопоставляет засеянные карточки с пришедшими по content key — по файлу, под которым релиз публикует рисунок, — и переписывает состояния карточек, ответы, незаконченную сессию и очередь отправки на идентификаторы текущего релиза (#404). Не сопоставляется только карточка, которую пришедший релиз не публикует: её записи остаются на устройстве нетронутыми, но в счётчики больше не попадают, и об этом **говорят** — на вкладке «Прогресс» появляется разовое сообщение с числом таких карточек. Настоящее решение — выводимые стабильные идентификаторы контента на бэкенде; тогда переносить будет нечего.
