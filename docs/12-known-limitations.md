...
- **Прогресс, набранный до первой успешной синхронизации, её не переживает.** Приложение засевает пустое хранилище зашитым каталогом ([ADR-021](./adr/ADR-021-bundled-catalogue-snapshot.md)), но идентификаторы карточек в нём выведены из content key, а сервер выдаёт свои. Первая же успешная синхронизация целиком вытесняет засев, и локальные записи прогресса начинают указывать на карточки, которых нет в текущем релизе: в счётчиках они не видны (счёт джойнится с текущим релизом), а сессия из них отклоняется при импорте как `OFFLINE_SESSION_COMPOSITION_INVALID`. Гостевая работа наверх не уходит вовсе, так что чужому аккаунту ничего не приписывается. Чинится только на бэкенде — выводимыми стабильными идентификаторами контента.
