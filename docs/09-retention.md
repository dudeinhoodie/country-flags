...
| `Device.deletedAt` | Удалённое пользователем устройство хранится до удаления аккаунта | ✅ By design | `DevicesService.delete` проставляет `deletedAt`, очищает `pushTokenEncrypted` и отзывает сессии устройства; строка остаётся, потому что на неё ссылаются неизменяемые `review_events`. Удаляется вместе с аккаунтом (`account-deletion.service.ts`, после review events). |
