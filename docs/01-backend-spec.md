...
- У предела `DUE_ONLY` возвращает пустую сессию. `STANDARD` продолжает раздавать новые карточки и карточки, чей срок ещё не наступил: заниматься сверх дня можно, но такое занятие не выдаётся за плановую очередь (#431). Поэтому сессия у предела может содержать меньше карточек, чем запрошено, или ни одной.
- Приём ответов лимит **не** применяет: ни `POST /v1/reviews/batch`, ни импорт офлайн-сессии, ни guest import. Ответ — исторический факт (§8.3), и отказ в нём потерял бы честно сделанную работу; сессия, собранная устройством без сети, импортируется целиком. Такие ответы расходуют день, как любые другие.
- `due-summary` и progress считают due-карточки по той же отсечке: одна очередь дня на аккаунт, и колоды с регионами показывают свою долю в ней.
