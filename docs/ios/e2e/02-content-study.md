...
- Раскрыть первую карточку и почти одновременно дважды нажать `Again` либо
  ...
  **Expected result**
  ...
  - карточка планируется на повтор планировщиком, а не текущей сессией:
    `.again` даёт час и состояние `RELEARNING`, поэтому повтор приходится
    на следующий присест.
...
- Ответить на остальные четыре уникальные карточки.
  ...
  **Expected result**
  ...
  - session завершается на пятой карточке: состав зафиксирован на старте,
    и `Again` его не удлиняет;
  - planned unique count остаётся равным 5;
  - на экране результата отказанная карточка попадает в «вернутся
    в колоду» и не считается запомненной.

- Завершить сессию.

  **Expected result**

  - result показывает 5 unique cards и 5 фактических review: повтора внутри
    присеста нет, поэтому шестого review взяться неоткуда;
  - отказанная карточка записана как `RELEARNING` и запланирована на час
    вперёд, то есть вернётся следующим присестом, а не этим;
