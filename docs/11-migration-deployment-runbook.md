...
## 2a. Откат релиза контента

Откат контента не связан с откатом образа: релиз — это активный указатель
`ContentPointer`, и вернуть его на предыдущую версию можно, не трогая ревизию.

Из консоли (обычный путь, ADR-017): `/releases` → `Roll back` → выбрать версию,
которую это окружение действительно публиковало. Роль `PUBLISHER` или `ADMIN`; в
production дополнительно требуется ввести версию вручную. Прогон записывается и
исполняется publisher-job'ом; экран показывает стадию и результат, и переживает
закрытие вкладки.

Из CLI (аварийный путь, когда консоль недоступна):

~~~bash
DATABASE_URL=... DIRECT_DATABASE_URL=... \
OBJECT_STORAGE_PROVIDER=s3 OBJECT_STORAGE_BUCKET=... \
corepack yarn content:bundle:rollback --to-version <version>
~~~

Откат не пересобирает бандл: релиз уже опубликован и подписан, поэтому ключ
подписи не нужен — читается архив `content-bundles/<version>/` и проверяется по
контрольным суммам манифеста. Обе дороги берут advisory-lock на активном
указателе, так что одновременная публикация не может обогнать откат.
