...
- На первом запуске приложение засевает пустое хранилище каталогом того же релиза, который зашит в бандл, — до любого сетевого запроса, так что колоды и карточки есть даже без сети; см. [ADR-021](./adr/ADR-021-bundled-catalogue-snapshot.md). Засев происходит только при пустом хранилище, лежит под собственной версией `bundled:<release>` и целиком вытесняется первой успешной синхронизацией.
...
- Флаги одного релиза зашиты в приложение и рисуются, когда checksum опубликованного представления совпадает с зашитым; см. [ADR-011](./adr/ADR-011-bundled-flag-baseline.md).
- Остальные assets загружаются лениво, но флаги выбранной сессии префетчатся.
...
`templateCode + schemaVersion` отсеивается при сборке сессии и в новую сессию не
попадает, поэтому нарисовать герб flag-рендерером невозможно.

Сессия, собранная не этим билдом — возобновлённая после обновления или
выбранная бэкендом, — может содержать такую карточку. Она получает unsupported
state вместо чужого рендерера, остаётся оцениваемой и не блокирует сессию. Оба
случая уходят в operational-отчёт `unsupported_card_template` (§12 документа
18) один раз на пару.
...
Визуальный язык, референсы, motion, haptics и словарь нативных компонентов
вынесены в [16-ios-design-language.md](./16-ios-design-language.md). Требования
ниже обязательны; документ дизайна их не ослабляет.
...
- Язык контента подбирается по языку, который разрешил localization catalog интерфейса (`Bundle.preferredLocalizations`), а не по сырому списку языков устройства: на устройстве с языком, которого нет в приложении, интерфейс и контент откатываются на один и тот же язык.
