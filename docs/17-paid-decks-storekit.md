...
Реализация gate: переменная окружения `PAID_CONTENT_MINIMUM_CLIENT_VERSIONS`
вида `ios=1.4.0` — первая сборка каждой платформы, которая понимает
`Deck.access`. Сервер читает заголовки `X-Client-Platform` и
`X-Client-App-Version`, которые клиент шлёт на каждом запросе. Сборка ниже
порога — как и любой вызов без заголовков — получает каталог, который видела
всегда: `GET /v1/decks` отдаёт только free-колоды, `GET /v1/decks/{id}` на
платную отвечает 404, `GET /v1/content/changes` не анонсирует ни платную
колоду, ни её preview-материал; `RETIRE` доходит всегда. Отказа в запросе
нет: бесплатное приложение продолжает работать. Пустая переменная означает,
что подходящих сборок ещё нет, — платные колоды не видит никто. Порог
поднимается на месте (`gcloud run services update --update-env-vars`), без
пересборки образа. Метрика `client_version_gate_total{route,outcome}`
показывает долю трафика, которая пока не обновилась.
