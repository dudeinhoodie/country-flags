...
| Entitlement | Стабильное право доступа, например `deck.europe_coats` |
...
~~~text
offer code:          EUROPE_COATS_LIFETIME
entitlement key:     deck.europe_coats
production product:  app.countryflags.deck.europe_coats.lifetime.v1
dev product:         app.countryflags.dev.deck.europe_coats.lifetime.v1
~~~
...
~~~json
{
  "items": [
    {
      "code": "EUROPE_COATS_LIFETIME",
      "kind": "ONE_TIME",
      "storeProduct": {
        "provider": "APPLE_APP_STORE",
        "productId": "app.countryflags.deck.europe_coats.lifetime.v1"
      },
      "grants": ["deck.europe_coats"]
    }
  ]
}
~~~
...
К существующему `Deck` (`contracts/openapi/components.yaml:960`) добавляется обязательный для нового клиента объект:
...
- `POST /v1/study-sessions` проверяет entitlement перед созданием новой server session.
...
Добавить flag `monetization.paid_decks.storefront_enabled`, default `false`:

- управляет показом кнопки покупки и rollout storefront;
...
- backend access guard работает независимо от flag.
...
~~~text
EUROPE_COATS_LIFETIME          ACTIVE

Type:        One-time purchase
Grants:      deck.europe_coats
Apple:       app.countryflags.deck.europe_coats.lifetime.v1
...
- feature flag по умолчанию false во всех средах до завершения rollout gate.
...
1. Добавить backend schema/API/verification с storefront flag `false`.
...
8. Включить storefront только в dev, затем небольшой production rollout.
