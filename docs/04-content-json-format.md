...
- клиент выбирает первое представление, которое умеет декодировать, поэтому неизвестная ему кодировка безвредна, а не фатальна. iOS не декодирует SVG из скачанных байтов и рисует растр;
...
- поля `path`/`mimeType`/`sha256` верхнего уровня описывают вектор и сохраняются один релиз ради клиентов, которые появились до `representations`.
