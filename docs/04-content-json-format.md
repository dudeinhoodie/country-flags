...
Отсюда два редакционных правила. Ключ начинается с буквы: `deck.1990s` даёт код,
который контракт не примет. И два ключа не должны сходиться в один код —
...
Колода, которой нет в публикуемом релизе, снимается с публикации: строка
получает статус `RETIRED`, исчезает из `GET /v1/decks` и `GET /v1/decks/{id}`, а
в ленту изменений релиза попадает `RETIRE` для неё. Строка не удаляется —
состав колоды, её сессии и прогресс по ней (`user_deck_mastery`) сохраняются, и
релиз или откат, в котором колода снова есть, публикует ту же строку. Publisher
сравнивает колоды по коду, а не по ключу, и снимает каждую опубликованную
редакционную колоду, которой нет в релизе, — в том числе оставшуюся от старого
релиза (#440). Пользовательские колоды (`DYNAMIC_USER`, `CUSTOM`) релиз не
трогает.
...
### 8.3 Факты, которые называет редактор

Часть фактов приходит из источников, часть не приходит ниоткуда: дата
вступления штата в союз, девиз, крупнейший город. Такие значения пишутся в
`facts` сущности — в том же виде, в каком релиз их публикует:
...
~~~json
{
  ...
  "type": "subdivision",
  "facts": {
    "capitals": [{ "names": { "en": "Sacramento", "ru": "Сакраменто" } }],
    "statehoodDate": { "date": "1850-09-09" },
    "largestCity": { "names": { "en": "Los Angeles", "ru": "Лос-Анджелес" } }
  }
}
~~~

Именно `facts`, а не `overrides`. Оверрайд — это точечный путь в собранную
запись, и он ничего не знает о том, читает ли его кто-нибудь: факт,
записанный туда, сохраняется, публикуется и молча теряется. Значения из
`facts` попадают в сборку по типу факта и всегда оказываются в
опубликованной коллекции с редакционным приоритетом.

Коллекция типа, который приходит только от редактора, публикуется лишь
тогда, когда хотя бы одна сущность его называет: двести пропусков, не
говорящих ничего, не нужны никому.
...
### 12.1 Представления ассета

`assets.json` публикует для каждого ассета список `representations` — все кодировки одного изображения в порядке предпочтения клиента: сначала векторный оригинал, затем растр по возрастанию `scale`.

```json
{
  "key": "flag.france.current",
  "entityKey": "country.france",
  "representations": [
    { "path": "assets/svg/france.svg", "mimeType": "image/svg+xml", "sha256": "…" },
    {
      "path": "assets/png/france@2x.png",
      "mimeType": "image/png",
      "sha256": "…",
      "scale": 2,
      "widthPx": 320,
...
- клиент выбирает представление под масштаб своего экрана, а не первое подходящее: точное совпадение `scale`, иначе ближайшее меньшее, иначе первое, что вообще декодируется. Неизвестная кодировка безвредна, а не фатальна. iOS не декодирует SVG из скачанных байтов и рисует растр;
- `sha256` принадлежит представлению, а не ассету: клиент проверяет те байты, которые скачал, и контрольная сумма вектора не отвечает за растр;
- растр публикуется в `@2x` и `@3x` от базовой высоты 120pt — столько занимает флаг в самом крупном месте, в prompt карточки; списки стран и варианты ответов масштабируют тот же файл вниз;
- растр рендерится из уже санитизированного вектора, поэтому не может показать то, чего нет в оригинале;
- релиз, в котором хотя бы один ассет не публикует растр, отклоняется валидатором bundle: такой флаг не нарисует ни один клиент;
- `representations` — единственное место, где описана кодировка. До версии контракта 2.0.0 ассет дублировал вектор в собственных `path`/`mimeType`/`sha256` ради читателей, появившихся раньше списка; таких читателей не выпустили, и поля удалены.
